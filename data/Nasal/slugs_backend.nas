...
var BEAD_DIST = 10;			# leg length in meters
...
var draw_L2_vector = func() {
	...
	l2_count = floor1(dist / L2_LENGTH); #number of line pieces needed
	
	setsize(l2_boxes, l2_count);
	
	if (L2_DEBUG_PRINT)
		print("Drawing " ~ l2_count~ " l2 pieces.");
	...
	for( var i = 0; i < l2_count; i += 1) {
		l2_boxes[i] = geo.put_model(L2_MARKER, curr_pos, hdg + L2_ANGLEOFFSET);
...
# Plots most recently added waypoint as aim point
var draw_aim_point = func() {
	wp_num = getprop("/autopilot/route-manager/route/num");	
	
	if (wp_num <= 1)
		die("No waypoints loaded for drawing aim point.");
	...
	ap_wp.set_latlon(lat,lon,alt);
	
	print("Drawing aim point (" ~ ap_wp.lat() ~ "," ~ ap_wp.lon() ~ "," ~ ap_wp.alt() ~ ").");
	ap_bead = geo.put_model(AP_MARKER, ap_wp, 0); # heading used to be offset by 90
}

# Plots most recently added waypoint as initial point
...
var draw_leg = func(wp1, wp2) {
	...
	# Connect end to beginning with leg
	#if (CONNECT_ROUTE_BEGIN_TO_END) {
	#}
...
var init = func() {
	# remove sides (top already removed by glide_slope_tunnel script)
	var left = props.globals.initNode("/sim/model/geometry/square/left", 1, "BOOL");
	var right = props.globals.initNode("/sim/model/geometry/square/right", 1, "BOOL");
}
